...
//#include <sendproxy> - Removed due to being outdated. Crashes server if enabled.

//#pragma newdecls required //i have no idea why but buttons.sp freaks out if you keep this 
...
char g_sRoles[][] = { "NOROLE", "{lime}Inocente{default}", "{dodgerblue}Detetive{default}", "{fullred}Traidor{default}", 
"{yellow}Farsa{default}", "{mediumpurple}Necromante{default}", "{black}Pestilência{default}", "{strange}Trovão{default}"};
...
//traitor glow
int   g_iGlowSprite   = -1;
Handle g_hGlowTimer   = INVALID_HANDLE;

public Plugin myinfo = 
{
	name = "[TF2] Trouble In Terrorist Town", 
	author = "yelks, what.rf", 
...
{
	//register actual glow
	g_iGlowSprite = PrecacheModel("sprites/redglow1.vmt", true);
...
public Action Timer_Hud(Handle timer)
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			...
			{
				huds[0].DisplayClient(i, "Round pendente");
			}
			else if (!IsPlayerAlive(i) && g_eRound == Round_Active)
			{
				huds[0].DisplayClient(i, "Você está morto");
			}
			else if (role >= TRAITOR)
			{
				if (role == PESTILENCE)
				{
					...
					{
						huds[0].DisplayClient(i, "Pestilência");
					}
				}
				else
				{
					huds[0].DisplayClient(i, "Traidor");
				}
			}
			else if (role == DETECTIVE)
			{
				huds[0].DisplayClient(i, "Detetive");
			}
			else if (role == INNOCENT)
			{
				huds[0].DisplayClient(i, "Inocente");
			}
		}
	}
	return;
...
public void OnClientDisconnect_Post(int client)
{
	...
	{
		CPrintToChatAll("%s O Último Traidor saiu do servidor! Os Inocentes ganham!", TAG);
		ForceTeamWin(2);
	}
	else if (innoCount == 0 && traitorCount > 0)
	{
		CPrintToChatAll("%s O Último Inocente saiu do servidor, os Traidores ganham!", TAG);
		ForceTeamWin(3);
	}
}
...
public Action Listener_JoinTeam(int client, const char[] command, int args)
{
	...
	CPrintToChat(client, "%s Aguarde o round atual acabar.", TAG);
	return Plugin_Handled;
}
...
public Action Listener_JoinClass(int client, const char[] command, int args)
{
	...
	if (g_eRound == Round_Active)
	{
		CPrintToChat(client, "%s Você não pode trocar de classe durante o round.", TAG);
		return Plugin_Handled;
	}
...
public void Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
{
	if (IsValidClient(i) && hMap[i] == INVALID_HANDLE)
	{
		PrintToServer("[TTT DEBUG] WARNING: hMap[%d] is uninitialized! Initializing now...", i);
		hMap[i] = new StringMap();
	}
}
...
public Action Event_PlayerDeathPre(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (traitorCount == 0 && innoCount > 0)
	{
		CPrintToChatAll("%s Todos os Traidores morreram, os Inocentes ganham!", TAG);
		ForceTeamWin(2);
	}
	else if (innoCount == 0 && traitorCount > 0)
	{
		CPrintToChatAll("%s Os Inocentes morreram, os Traidores ganham!", TAG);
		ForceTeamWin(3);
	}
	...
	if (pAttacker.role >= TRAITOR && pVictim.role < TRAITOR) // TRAITOR KILLS INNO/DET
	{
		...
		{
			CPrintToChat(attacker, "%s Você pode usar o {fullred}INSTANT KILL{default} com sua arma corpo a corpo!", TAG);
		}
	}
...
public void OnRoundEnd(const char[] output, int caller, int activator, float delay)
{
	...
	CPrintToChatAll("%s Os Inocentes sobreviveram, eles ganham!", TAG);
	ForceTeamWin(2);
}
...
public void Event_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			...
			{
				CPrintToChatAll("%s {red}%N {default}(%s) matou {purple}%i{default} jogadores.", TAG, i, g_sRoles[player.role], player.killCount);
...
public Action OnClientSayCommand(int client, const char[] command, const char[] message)
{
	...
	if (!IsPlayerAlive(client))
	{
		if (TTTPlayer(client).role == NECROMANCER && StrEqual(message, "respawn", false))
		{
			...
			{
				CPrintToChat(client, "%s Aguarde %0.1f segundos antes de reviver outros traidores.", TAG, g_cvRespawnDelay.IntValue - (now - g_fLastAbility[client]));
				return Plugin_Handled;
			}
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if (g_eRound == Round_Active && IsValidClient(victim) && IsValidClient(attacker) && victim != attacker)
	{
		...
		if (pAttacker.role == TRAITOR && IsMeleeActive(attacker) && pAttacker.killCount >= 3)
		{
			...
			CPrintToChat(attacker, "%s Você usou seu INSTANT KILL.", TAG);
			damage = 9999.0;
...
{	if (g_hGlowTimer != INVALID_HANDLE)
    {
        ...
        PrintToServer("[TTT Debug] GlowTimer killed.");
    }
}
/* Debug output
==================================================================================================== */
