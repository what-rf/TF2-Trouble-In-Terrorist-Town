...
void StartTTT()
{
	...
	while (traitorRequired > 0 && g_aForceTraitor.Length > 0)
	{
		...
		if (IsValidClient(client))
		{
			...
			player.role = role;
			player.credits += 2;
			CPrintToChat(client, "%s {community}Você é o %s.", TAG, g_sRoles[role]);
			CPrintToChat(client, "%s {fullred}Você pode usar o teamchat para falar com outros traídores.", TAG);
			traitorRequired--;
		}
	...
	while (detectiveRequired > 0 && g_aForceDetective.Length > 0)
	{
		...
		if (IsValidClient(client))
		{
			...
			CPrintToChat(client, "%s {community}Você é um %s.", TAG, g_sRoles[DETECTIVE]);
			CPrintToChat(client, "%s {community}Você tem %i karma. Você causa %i%% de dano.", TAG, player.karma, player.karma);
			detectiveRequired--;
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			...
			{
				CPrintToChat(i, "%s {community}Você é um %s.", TAG, g_sRoles[INNOCENT]);
				CPrintToChat(i, "%s {community}Você tem %i karma. Você causa %i%% de dano.", TAG, player.karma, player.karma);
			...
			{
				//SetEntProp(i, Prop_Send, "m_bGlowEnabled", 1); //old sendproxy jank
			...
			//SendProxy_Hook(i, "m_bGlowEnabled", Prop_Int, SendProxy_Glow); //old sendproxy jank
			player.Setup();
		}
	}
...
void AssignTraitors(const int required)
{
	...
	for (int i = 0; i < required; i++)
	{
		...
		if (random != -1)
		{
			...
			CPrintToChat(random, "%s {community}Você é o %s.", TAG, g_sRoles[player.role]);
			CPrintToChat(random, "%s {fullred}Você pode usar o teamchat para falar com outros traídores.", TAG);
		}
	}
...
//public Action SendProxy_Glow(int iEntity, const char[] cPropName, int &iValue, int iElement)
//{
//    if (!g_cvEnabled.BoolValue)
//        return Plugin_Continue;

    // Only traitors glow (for now, to everyone)
//    if (TTTPlayer(iEntity).role >= TRAITOR)
//    {
//        iValue = 1;
