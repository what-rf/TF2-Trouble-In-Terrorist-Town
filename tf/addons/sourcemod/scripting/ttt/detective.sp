...
void PerformScan(int client)
{
	...
	{
		CPrintToChat(client, "%s Aguarde %0.1f segundos antes de usar o scanner de novo.", TAG, g_fLastAbility[client] + g_cvScannerDelay.IntValue - now);
		return;
	}
...
Action ScanPlayer(Handle timer, int client)
{
	...
	ShowHudText(client, 5, "Escaneando %N\n%s", target, szBuffer);
	ShowHudText(target, 5, "Você está sendo escaneado!\n%s", szBuffer);
