...
void PerformEarthquakes()
{
	...
	CPrintToChatAll("%s {fullred}CUIDADO:{default} O %s criou terremotos!", TAG, g_sRoles[NECROMANCER]);
}
...
void Necromancer_OnRunCmd(const TTTPlayer player, int button)
{
	...
	if (button == IN_ATTACK2)
	{
		...
		{
			CPrintToChat(client, "%s Aguarde %0.1f segundos antes de criar mais terremotos.", TAG, g_fLastAbility[client] + g_cvEarthquakeDelay.IntValue - now);
			return;
		}
