...
void OpenShop(const TTTPlayer player)
{
	if (player.role == TRAITOR)
	{
		...
		menu.SetTitle("Shop Menu\nVocê tem %i créditos", player.credits);
...
{  
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			{
				CPrintToChat(client, "%s Você não tem créditos suficientes para comprar isso.", TAG);
				return;
			}
...
{  
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			{
				CPrintToChat(client, "%s Você já comprou para um cargo.", TAG);
				return;
			}

			if (player.role != NOROLE)
			{
				CPrintToChat(client, "%s Você só pode comprar isso durante o setup.", TAG);
				return;
			}
